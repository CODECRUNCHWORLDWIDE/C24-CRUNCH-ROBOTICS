...
```mermaid
flowchart LR
  LLM["LLM planner"] --> Sig["Skill signature grasp of object"]
  Sig --> Impl["Implementation swappable"]
  Impl --> A["Analytic grasp"]
  Impl --> B["Learned VLA grasp"]
  Impl --> C["MoveIt2 plan"]
```
*The LLM plans against the signature only; the implementation behind it can be swapped without changing a single plan.*
...
```mermaid
flowchart TD
  A["Perceive detect objects"] --> B["Prompt planner with instruction and world"]
  B --> C["LLM emits constrained plan"]
  C --> D["Ground validate against world"]
  D -->|Ungrounded| E["Repair or reject"]
  E --> B
  D -->|Grounded| F["Execute skill by skill"]
  F --> G["Re observe world"]
  G -->|Effect holds| F
  G -->|World changed or failed| B
```
*The instruction-to-plan loop, looping back to re-plan when grounding or execution fails.*
