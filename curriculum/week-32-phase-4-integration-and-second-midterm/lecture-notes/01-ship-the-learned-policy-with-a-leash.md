...
stateDiagram-v2
    ...
    One --> Two: Reject
    Two --> Three: Reject
    Zero --> Zero: Safe action
    One --> Zero: Safe action
    Two --> Zero: Safe action
    Three --> Fallback: BT switches to classical planner
    Fallback --> Zero: Counter resets after completion
```
*The rejection counter climbs on each rejected action but resets on any safe action, firing the fallback only after three in a row.*
...
```mermaid
flowchart TD
    A["Perception and grasp pipeline"] --> B["Learned policy proposes action"]
    B --> C["Safety filter clamps projects or rejects"]
    C -->|"Accepted"| D["Controller executes"]
    C -->|"Three rejections"| E["Classical fallback planner"]
    F["Behavior tree"] --> B
    F --> E
```
*The five Phase 4 nodes compose in a fixed safety order, with the behavior tree dispatching the learned-policy branch and the classical-fallback branch.*
