...
sequenceDiagram
  ...
  A->>B: SPDP announcement multicast
  B->>A: SPDP announcement multicast
  Note over A,B: Both now know each other exists
  B->>A: SEDP unicast topic type and QoS
  A->>A: Check request offered rule
  alt QoS compatible
    A->>B: Data flows
  else QoS incompatible
    A->>A: Connection silently not formed
  end
```
*SPDP finds participants over multicast; SEDP then exchanges QoS over unicast, where compatibility is actually decided.*
...
```mermaid
flowchart TD
  A["Subscriber gets nothing"] --> B{"Does ros2 topic list show the topic"}
  B -->|No| C["Discovery problem: check domain id, multicast, rmw implementation"]
  B -->|Yes| D{"Does topic info show both a publisher and subscriber"}
  D -->|No| E["Other node not running or topic name typo"]
  D -->|Yes| F{"Do publisher and subscriber QoS blocks match"}
  F -->|No| G["QoS mismatch: apply the request offered rule"]
  F -->|Yes| H{"Do type hashes match"}
  H -->|No| I["Message version mismatch: rebuild both ends"]
  H -->|Yes| J["Data flows but wrong: check frame id and stamp"]
```
*Walking the tree from no data to root cause: discovery, QoS, version, then message semantics.*
