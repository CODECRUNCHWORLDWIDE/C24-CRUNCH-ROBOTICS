...
```mermaid
flowchart TD
  R["robot.launch.py top level"] --> D["description.launch.py"]
  R --> G["gz_sim.launch.py"]
  R --> S["slam.launch.py"]
  R --> V["rviz.launch.py"]
```
*The top-level file composes named subsystem launch files rather than declaring nodes itself.*
...
Three broadcasters, three disjoint sets of edges, one connected tree rooted at `map`. No edge has two owners.

```mermaid
flowchart TD
  M["map"] -->|slam_toolbox| O["odom"]
  O -->|Gz DiffDrive plugin| B["base_link"]
  B -->|robot_state_publisher| L["laser"]
  B -->|robot_state_publisher| I["imu"]
  B -->|robot_state_publisher| W["left_wheel"]
```
*One broadcaster owns each TF edge in the tree rooted at map.*

When the reviewer asks "who publishes `odom → base_link`?", the answer is one node, named, and you can prove there is no second one with:
