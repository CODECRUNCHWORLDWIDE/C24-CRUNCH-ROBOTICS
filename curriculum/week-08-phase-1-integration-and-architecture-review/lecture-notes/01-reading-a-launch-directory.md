...
```mermaid
flowchart TD
  A["Find the entry point"] --> B["Read declared arguments"]
  B --> C["Trace includes into a tree"]
  C --> D["Resolve substitutions by hand"]
  D --> E["Reconstruct the runtime graph"]
  E --> F["Verify against ros2 node list and view frames"]
```
*The five-step algorithm for reading an unfamiliar launch directory.*
...
sequenceDiagram
  ...
  participant Gz as Gz Server
  participant Spawn as Spawn Robot

  Launch->>Gz: start process
  Gz-->>Launch: OnProcessStart fires
  Launch->>Spawn: start spawn robot
  Spawn->>Gz: create robot_description
```
*The spawn action waits for the OnProcessStart event instead of racing the server.*
