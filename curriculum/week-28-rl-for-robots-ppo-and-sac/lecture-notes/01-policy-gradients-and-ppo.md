...
```mermaid
flowchart LR
  S["State s"] --> Actor["Actor pi theta"]
  S --> Critic["Critic V phi"]
  Actor --> A["Action a"]
  Critic --> B["Baseline value"]
  A --> Adv["Advantage: return minus baseline"]
  B --> Adv
  Adv -.->|scales the gradient| Actor
```
*The actor picks actions while the critic's baseline turns raw return into a low-variance advantage signal.*
...
```mermaid
flowchart TD
  A["Collect rollout under old policy"] --> B["Compute GAE advantages and returns"]
  B --> C["Run several epochs of clipped minibatch updates"]
  C --> D["Discard the batch"]
  D --> A
```
*PPO's on-policy cycle: gather data, reuse it briefly under the clip, then throw it away.*
