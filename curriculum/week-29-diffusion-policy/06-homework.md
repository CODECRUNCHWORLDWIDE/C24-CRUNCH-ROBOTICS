...
**Hint.** A reviewer trusts the *controlled* comparison and the *measured* latency. "Diffusion Policy scored 0.91 vs BC's 0.58 on 50 fixed-seed episodes; the scatter shows why — BC's mean lands in the obstacle" reads like an engineer; "Diffusion Policy worked better" reads like a student.
...
When you've finished all six, push your repo and make sure the Diffusion Policy, its eval harness, and the ROS2 node are in the same workspace — Week 30 compares ACT against them on the same measuring stick. Then take the [quiz](./05-quiz.md) with your notes closed.
