...
```mermaid
flowchart LR
    O["Observation history"] --> OE["Observation encoder"]
    OE --> C["Conditioning vector"]
    T["Diffusion timestep"] --> TE["Sinusoidal time embedding"]
    TE --> C
    A["Noised action chunk"] --> U["Denoiser FiLM conditioned"]
    C --> U
    U --> P["Predicted noise"]
```
*Observation and timestep are encoded into one conditioning vector that steers the denoiser; only the action chunk is ever noised.*
...
```mermaid
flowchart LR
    A["Observe current state"] --> B["Predict action chunk via denoising"]
    B --> C["Load chunk into queue"]
    C --> D["Execute first Ta actions"]
    D --> A
```
*Plan long, commit short, re-plan often: the receding-horizon loop that turns a chunk predictor into a reactive controller.*
...
Next: the exercises put DDPM, DDIM, and a full Diffusion Policy in your hands, the challenge stages the BC-vs-diffusion multimodal showdown, and the mini-project trains the real thing on your Week-27 demos and deploys it. Continue to [the exercises](../exercises/README.md).
