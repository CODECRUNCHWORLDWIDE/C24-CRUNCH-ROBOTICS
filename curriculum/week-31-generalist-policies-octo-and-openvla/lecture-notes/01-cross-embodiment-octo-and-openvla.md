...
```mermaid
flowchart TD
  A["RT-1 2022 - token prediction on one robot"] --> B["RT-2 2023 - pretrained VLM plus robot data"]
  B --> C["RT-X 2023 - trained on pooled OXE data"]
  C --> D["Octo 2024 - open small transformer generalist"]
  C --> E["OpenVLA 2024 - open Prismatic VLM generalist"]
```
*The RT lineage that established positive cross-embodiment transfer, and the open re-creations you'll actually run.*
...
```mermaid
flowchart LR
  A["Camera image"] --> B["DINOv2 - geometry features"]
  A --> C["SigLIP - semantic features"]
  B --> D["Fused visual features"]
  C --> D
  D --> E["Projection layer"]
  F["Instruction text"] --> G["Llama-2-7B"]
  E --> G
  G --> H["7 action tokens"]
```
*How the fused DINOv2 plus SigLIP visual features and the instruction flow through the Prismatic backbone to produce action tokens.*
