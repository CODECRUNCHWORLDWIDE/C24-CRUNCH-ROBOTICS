...
```mermaid
flowchart TD
  F1["LiDAR dropout"] --> C1["Fail safe state: retract then stop"]
  F2["Controller goes unstable"] --> C2["Fail stop: halt immediately"]
  F3["E-stop pressed"] --> C2
```
*Which fault maps to which fail-safe category: trusted actuators may retract, suspect ones just halt.*
...
```mermaid
flowchart LR
  A["Latch published"] --> B["Publish and transport"]
  B --> C["Detection: callback fires"]
  C --> D["Action cancel dispatch"]
  D --> E["Actuator response: stop"]
```
*The 200 ms budget is spent across four hops, not paid all at once.*
...
You have composed the robot and put a measured leash on it. The exercises stand it up; the mini-project runs it once and signs the milestone. Continue to [the exercises](../exercises/README.md).
