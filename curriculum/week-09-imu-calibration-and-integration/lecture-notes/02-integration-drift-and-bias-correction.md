...
stateDiagram-v2
  [*] --> Calibrating
  Calibrating --> Calibrating : accumulate sample
  Calibrating --> Running : count reaches target and bias computed
  Running --> Running : subtract bias and publish
```
*The bias corrector node has exactly two phases: accumulate then run.*
...
```mermaid
flowchart TD
  A["Stationary log"] --> B["Allan variance"]
  B --> C["Noise density and bias instability"]
  A --> D["Stationary mean"]
  D --> E["Gyro bias and accel bias"]
  F["Live imu data"] --> G["ImuBiasCorrector"]
  E --> G
  C --> G
  G --> H["Calibrated imu data"]
  H --> I["Week 10 EKF"]
```
*The same pipeline redrawn as a flowchart: two data paths from the log feed one running node.*
...
Next: the exercises put the Allan plot and the bias node on your own data. Continue to [the exercises](../exercises/README.md).
