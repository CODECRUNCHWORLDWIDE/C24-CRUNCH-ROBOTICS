...
```mermaid
flowchart TD
  A["Accelerometer at rest"] --> B["Sees gravity vector"]
  B --> C["Roll and pitch observable"]
  C --> D["Bounded no drift"]
  A --> E["Rotation about vertical axis"]
  E --> F["Gravity direction unchanged in body frame"]
  F --> G["Yaw not observable from accel"]
  G --> H["Unbounded drift over time"]
```
*Why gravity bounds roll and pitch but leaves yaw to drift freely.*
...
```mermaid
flowchart TD
  A["IMU error term"] --> B{"Observable from a stationary log"}
  B -->|Yes| C["Bias"]
  B -->|Yes| D["Noise via Allan variance"]
  B -->|Needs motion| E["Scale factor"]
  B -->|Needs motion| F["Misalignment"]
  E --> G["Turntable known angle test"]
  F --> H["Six position test"]
```
*Which error terms a stationary log can reveal versus which require motion to see.*
