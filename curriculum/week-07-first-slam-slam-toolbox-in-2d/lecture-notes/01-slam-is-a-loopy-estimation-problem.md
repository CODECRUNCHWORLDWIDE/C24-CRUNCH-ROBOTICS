...
flowchart LR
  ...
  X6 -.->|"loop closure edge"| X0
```
*The sequential edges form the trajectory backbone; the loop-closure edge ties the latest node back to an earlier one and makes the graph loopy.*
...
```mermaid
flowchart TD
  A["New node created"] --> B["Candidate search near current pose"]
  B --> C["Chain match against candidate chain"]
  C --> D{"Response above threshold"}
  D -->|"Yes"| E["Accept loop closure edge"]
  D -->|"No"| F["Reject - no edge added"]
```
*How slam_toolbox decides whether a revisit becomes a loop-closure edge.*
