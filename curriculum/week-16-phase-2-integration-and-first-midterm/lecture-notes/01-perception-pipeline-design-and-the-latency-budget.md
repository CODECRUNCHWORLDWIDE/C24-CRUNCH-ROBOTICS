...
flowchart LR
  IMU["IMU"] --> EKF["EKF"]
  WHEEL["Wheel odom"] --> EKF
  EKF --> CLUSTER["3D clustering"]
  LIDAR["LiDAR RGB-D cloud"] --> CLUSTER
  CLUSTER --> FUSION["Fusion data association"]
  CAM["Camera image"] --> YOLO["YOLO TensorRT"]
  YOLO --> FUSION
  FUSION --> OUT["perception objects topic"]
```
*IMU and wheel odometry feed the EKF, whose transforms let LiDAR clusters and YOLO detections fuse into one output.*
...
**"Inference time" is not "end-to-end latency."** The most common midterm failure is a learner who measured YOLO's inference at 12 ms and reports "12 ms perception." The panel asks "from the sensor stamp?" and the real number — acquisition + transport + inference + association + transform + publish — is 18–25 ms. The *hop you forgot* is the one the panel finds. The block diagram forces you to account for every hop, so there's no forgotten one.

```mermaid
flowchart LR
  ACQ["Camera acquisition 3ms"] --> TR1["Transport 1ms"]
  TR1 --> YOLO["YOLO inference 12ms"]
  CLOUD["Cloud acquisition"] --> CLUSTER["Clustering 8ms"]
  YOLO --> FUSION["Fusion 3ms"]
  CLUSTER --> FUSION
  FUSION --> PUB["Transform and publish 2ms"]
```
*The YOLO and clustering branches run in parallel, so the critical path is the slower branch plus fusion and publish, not the sum of every hop.*
