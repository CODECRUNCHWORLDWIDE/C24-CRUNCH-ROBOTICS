...
```mermaid
flowchart TD
  A["3D cluster from LiDAR"] --> C["Synchronize by stamp"]
  B["2D detection from YOLO"] --> C
  C --> D["Project cluster into image"]
  D --> E["Compute IoU matrix"]
  E --> F["Hungarian assignment"]
  F --> G["Matched pair fused object"]
  F --> H["Cluster only unknown class"]
  F --> I["Detection only logged"]
```
*Data association synchronizes the two streams, projects and matches them, then handles matched and unmatched cases explicitly.*
...
sequenceDiagram
  ...
  Panel->>Engineer: What is the block diagram
  Engineer->>Panel: Data flow inputs stages output
  Panel->>Engineer: What are the interface contracts
  Engineer->>Panel: Topic type frame rate QoS table
  Panel->>Engineer: What is the latency budget
  Engineer->>Panel: Measured p95 critical path
  Panel->>Engineer: What happens when inputs fail
  Engineer->>Panel: Robustness gates and degraded behavior
  Panel->>Engineer: Show the numbers
  Engineer->>Panel: Reproducible scripts and measurements
```
*The panel probes a perception stack in a fixed order: diagram, contracts, budget, failures, then numbers.*
...
The failure mode that ends midterms is *not* a weak component — it's a learner who can't produce these five things on demand. The learner who says "perception is fast and it works" and the learner who says "p95 is 28 ms, here's the budget, the critical path is the YOLO hop, the LiDAR-dropout case coasts on the EKF, here's the drift number" are equally good *engineers* — but only the second one passes the gate, because the gate grades the *defense*, not the code.
...
3. **Measure before you present, not during.** Run the latency probe and the drift measurement *before* the review and have the numbers (and the scripts) ready. A learner who measures live, under pressure, with a half-working graph, fails on stage. A learner who measured Saturday and brings the report defends calmly.

The midterm is a conversation you can rehearse, against a rubric you can read in advance, with a brief you write beforehand. The learners who fail treated it as a surprise; the learners who pass treated it as a contract — read the rubric, write the brief, measure the numbers, rehearse the questions. This week is the preparation; the review is the confirmation.
...
It helps to see the difference between a prepared defense and an improvised one, because the gap looks small the week before and decisive on review day. Two composite learners, drawn from past cohorts.
...
A counterintuitive but load-bearing point: **the most defensible posture in front of a panel is honesty about your stack's limits.** A learner who claims their perception is flawless invites the panel to find the flaw (they always can), and being caught in an overclaim is worse than the flaw itself. A learner who says "my drift is 0.18 m on this trajectory, but it degrades in long corridors where the ICP is degenerate — I haven't fused the IMU yet to constrain that, and that's my top Phase-3 action item" has *pre-empted* the panel's hardest question by naming the limit first.
...
One closing reframe to carry past this week: the midterm is not an obstacle the curriculum puts in your way — it is the curriculum *doing you a favor*, at the cheapest possible moment. By composing seven weeks of perception and forcing a defense, it surfaces every weak component, every dishonest covariance, every un-measured latency claim, while fixing them is still a one-week resubmission. The same weaknesses, undiscovered, become fatal at the capstone, where there's no resubmission and the stakes are graduation. So treat the gate as diagnostic, not adversarial: a failed defense that pinpoints "your EKF covariances are dishonest, your latency is un-measured, your detections are mis-framed" is a *gift* — it tells you exactly what to fix before it can hurt you. The learners who resent the gate fight it; the learners who use it walk into Phase 3 with a perception stack they know is sound, because the gate proved it. Be the second kind, and the first hard gate becomes the thing that makes the rest of the track possible.
...
Next: the exercises build the latency budget, the latency probe, and the data-association node; the challenge rehearses the panel defense; and the mini-project is the fused node, measured and defended. Continue to [the exercises](../exercises/README.md).
