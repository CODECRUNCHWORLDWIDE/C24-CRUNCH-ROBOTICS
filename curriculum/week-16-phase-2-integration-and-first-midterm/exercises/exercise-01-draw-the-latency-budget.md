...
This is the most important 90 minutes of the week, and it involves almost no robot code. You will turn your composed perception graph into the two artifacts that defend your entire midterm: an **interface-contract table** (every seam's topic/type/frame/rate/QoS) and a **latency block diagram** with a measured budget per hop and the critical-path total. Half of midterm failures are not engineering failures — they are *defense* failures, where the student can't produce the contract or the budget when the panel asks. This exercise is the insurance.
