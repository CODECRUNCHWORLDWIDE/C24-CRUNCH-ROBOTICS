...
```mermaid
flowchart LR
  A["Randomization config yaml"] --> B["Domain Randomizer sampler"]
  B --> C["One sampled world per episode"]
  C --> D["Applied to simulator"]
  D --> E["Policy trains in that world"]
```
*Sampler, policy, and simulator stay separate — only the applied world changes each episode.*
...
```mermaid
flowchart TD
  A["Train nominal policy on one world"] --> C["Evaluate both on held out world"]
  B["Train randomized policy on DR config"] --> C
  C --> D["Compute gap closed success difference"]
  D --> E["Check sanity line on training world"]
  E --> F["Report gap closure with confidence interval"]
```
*The honest evaluation pipeline: two policies, one held-out world, a sanity check, then the gap-closure number.*
...
Next: the exercises build the recipes, the config, and the gap metric on your own task. Continue to [the exercises](../exercises/README.md).
