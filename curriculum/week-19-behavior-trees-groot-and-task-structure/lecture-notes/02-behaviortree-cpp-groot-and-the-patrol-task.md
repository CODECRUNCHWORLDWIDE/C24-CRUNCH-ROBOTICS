...
```mermaid
sequenceDiagram
  participant Detector as IsPersonDetected
  participant Board as Blackboard
  participant Facer as FaceTowards
  Detector->>Board: write person_pos
  Facer->>Board: read person_pos
  Board-->>Facer: person location value
```
*Nodes never call each other directly — they read and write shared keys on the blackboard.*
...
```mermaid
flowchart TD
  A["Fallback patrol or retreat"] --> B["ReactiveSequence patrol with yield"]
  A --> C["Sequence retreat"]
  B --> D["Fallback handle person"]
  B --> E["KeepRunningUntilFailure patrol loop"]
  D --> F["Inverter IsPersonDetected"]
  D --> G["Timeout 60s wait for person to leave"]
  E --> H["Sequence one lap three waypoints"]
  C --> I["Navigate to charger then announce"]
```
*The full patrol tree: a reactive yield-and-wait branch nested inside a Fallback that only reaches retreat if the wait times out.*
