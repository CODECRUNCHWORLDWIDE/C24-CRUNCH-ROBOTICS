...
```mermaid
flowchart TD
  A["Pop lowest f from open set"] --> B{"Node already closed"}
  B -- yes --> A
  B -- no --> C["Mark node closed"]
  C --> D{"Node is goal"}
  D -- yes --> E["Reconstruct path"]
  D -- no --> F["Expand neighbors"]
  F --> G["Compute new cost to neighbor"]
  G --> H{"New cost is better"}
  H -- yes --> I["Update cost and push neighbor with f equals g plus h"]
  H -- no --> A
  I --> A
```
*The shared search loop behind both Dijkstra and A star, expanding the lowest-f node each step.*
...
stateDiagram-v2
  [*] --> Undiscovered
  Undiscovered --> Open : cost first found
  Open --> Closed : popped as lowest f
  Closed --> Open : inconsistent heuristic reopens
  Closed --> [*] : goal reached
```
*With a consistent heuristic a node never needs to leave the closed state once expanded.*
