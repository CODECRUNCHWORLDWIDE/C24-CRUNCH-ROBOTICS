...
```mermaid
flowchart LR
  Base["base_link"] --> J1["Joint 1 transform T01"]
  J1 --> J2["Joint 2 transform T12"]
  J2 --> Jdots["more joints in the chain"]
  Jdots --> Jn["Joint n transform"]
  Jn --> Tool["tool0"]
```
*Forward kinematics is the chain of per-joint transforms composed from base_link to tool0.*
...
```mermaid
flowchart TD
  J["Jacobian J"] --> SVD["SVD splits J into U Sigma V transpose"]
  SVD --> SV["Singular values sigma 1 through sigma 6"]
  SV --> Small["Smallest singular value near zero"]
  Small --> Mani["Manipulability w goes to zero"]
  Small --> Ellip["Manipulability ellipsoid flattens to a pancake"]
```
*The same Jacobian feeds the singular values, the manipulability number, and the ellipsoid picture.*
