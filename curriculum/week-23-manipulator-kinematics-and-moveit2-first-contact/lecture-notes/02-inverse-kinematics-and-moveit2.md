...
```mermaid
flowchart LR
  Goal["Pose or joint goal"] --> IK["Kinematics plugin solves IK"]
  IK --> Plan["OMPL planning pipeline"]
  Plan --> Smooth["Smoother and time parameterization"]
  Smooth --> Exec["Controller executes trajectory"]
  Exec --> Result["MoveItErrorCodes result"]
```
*The move_group pipeline from pose goal to executed trajectory, one stage per failure mode.*
...
```mermaid
flowchart TD
  Code["Read error_code.val"] --> Neg7["-7 NO_IK_SOLUTION"]
  Code --> Neg2["-2 PLANNING_FAILED"]
  Code --> Neg4["-4 CONTROL_FAILED"]
  Neg7 --> Reach["Check pose reachability"]
  Neg2 --> Scene["Check planning scene obstacles"]
  Neg4 --> Ctrl["Check controller status"]
```
*Each MoveIt2 error code routes debugging to a different subsystem instead of random edits.*
