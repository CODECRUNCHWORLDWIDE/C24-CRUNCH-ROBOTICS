...
```mermaid
flowchart TD
  A["Robot explodes on spawn"] --> B{"Does check_urdf pass"}
  B -- No --> C["Fix structural URDF errors"]
  B -- Yes --> D{"Explodes on first frame"}
  D -- Yes --> E["Cause 1 or 2 - inertia or mass"]
  D -- No --> F{"Static pose interpenetrates"}
  F -- Yes --> G["Cause 3 - self colliding geometry"]
  F -- No --> H["Cause 4 - degenerate joint"]
```
*The four-cause differential diagnosis for a robot that explodes on spawn.*
...
```mermaid
flowchart TD
  A["You write xacro source"] --> B["xacro expands to URDF"]
  B --> C["robot_state_publisher reads robot_description"]
  C --> D["rviz2 draws visual geometry"]
  C --> E["ros_gz_sim create spawns into Gz world"]
  E --> F["Gz Sim physics reads collision and inertial"]
  F --> G["DiffDrive plugin drives cmd_vel"]
  G --> H["Joint states publish back to robot_state_publisher"]
  H --> C
```
*One URDF source flows through xacro expansion into both the rviz2 visualization path and the Gz Sim physics loop.*
