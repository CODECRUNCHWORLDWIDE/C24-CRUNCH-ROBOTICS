...
```mermaid
flowchart LR
  A["Grid A in robotA map"] --> C["Transform cells to world"]
  B["Grid B in robotB map"] --> C
  C --> D["Compute merged extent"]
  D --> E["Fuse overlapping cells"]
  E --> F["Merged grid in world"]
```
*The three-step pipeline that turns two robot-local grids into one shared-frame grid.*
...
sequenceDiagram
  ...
  A->>M: publish current map
  B->>M: publish current map
  loop every 2 seconds
    M->>M: merge latest two maps
    M->>A: publish shared map
    M->>B: publish shared map
  end
```
*Each robot publishes on its own schedule; the merger fuses whatever it has on a timer, with no one blocking on anyone.*
...
Next: the exercises put all of this on two real robots in Gz Sim. Continue to [the exercises](../exercises/README.md).
