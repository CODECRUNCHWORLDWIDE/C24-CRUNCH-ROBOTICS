...
```mermaid
flowchart LR
  T["Two robots one graph"] --> C1["Topics collide"]
  T --> C2["TF frames collide"]
  T --> C3["Node names collide"]
  C1 --> F1["Namespace"]
  C2 --> F2["Frame prefix"]
  C3 --> F1
```
*Three collisions, three distinct fixes — mixing them up leaves half the problem unsolved.*
...
```mermaid
flowchart TD
  W["world"] --> AM["robotA map"]
  W --> BM["robotB map"]
  AM --> AO["robotA odom"]
  BM --> BO["robotB odom"]
  AO --> AB["robotA base link"]
  BO --> BB["robotB base link"]
```
*The shared world frame roots two separate, prefixed TF trees — one per robot.*
