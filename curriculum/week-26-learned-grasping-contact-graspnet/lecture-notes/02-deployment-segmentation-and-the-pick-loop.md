...
```mermaid
flowchart LR
  A["Color image"] --> B["Object detector YOLO or text grounded"]
  B --> C["Box or point prompt"]
  C --> D["SAM2 segmentation"]
  D --> E["Binary mask"]
  E --> F["Deproject masked depth"]
  F --> G["Object point cloud"]
```
*A 2D detection becomes a 3D object cloud, so the network only sees the target.*
...
```mermaid
flowchart TD
  A["Pre-grasp planned move"] --> B["Approach Cartesian move"]
  B --> C["Close gripper"]
  C --> D["Lift Cartesian move"]
  D --> E{"Object held"}
  E -->|"Yes"| F["Pick success"]
  E -->|"No"| G["Fail slip bucket"]
```
*The pick is a fixed sequence, not one motion, with an explicit slip check at the end.*
...
Next: the exercises put all of this on your week-23 arm and week-14 camera. Continue to [the exercises](../exercises/README.md).
