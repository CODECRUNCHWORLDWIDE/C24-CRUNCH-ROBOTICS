...
```mermaid
flowchart TD
  A["Structure tensor M at a pixel"] --> B{"Both eigenvalues large"}
  B -->|Yes| C["Corner, matchable in 2D"]
  B -->|No| D{"One large one small"}
  D -->|Yes| E["Edge, aperture problem"]
  D -->|No| F["Flat region, nothing to localize"]
```
*Classifying a pixel as corner, edge, or flat region from the two eigenvalues of the structure tensor.*
...
```mermaid
flowchart TD
  A["Pick minimal random sample"] --> B["Fit model to sample"]
  B --> C["Count inliers across all matches"]
  C --> D{"More samples to try"}
  D -->|Yes| A
  D -->|No| E["Keep model with most inliers"]
  E --> F["Refit using all its inliers"]
```
*RANSAC repeats fit-and-count over random minimal samples, then keeps and refits the best model.*
