...
```mermaid
flowchart LR
  A["Image and instruction"] --> B["Vision encoder"]
  B --> C["Visual tokens"]
  C --> D["Language model"]
  A --> D
  D --> E["Action tokens"]
  ...
  F --> G["Robot command"]
```
*A VLM backbone becomes a VLA by generating action tokens instead of a caption, then de-tokenizing them into a command.*
...
sequenceDiagram
  ...
  participant Gate
  participant MoveIt
  Camera->>VLA: image and instruction
  Camera->>OWLViT: image and phrase
  VLA-->>Gate: grasp target
  OWLViT-->>Gate: box and score
  Gate->>Gate: compare target to box
  alt targets agree
    Gate->>MoveIt: dispatch accepted grasp
  else targets disagree
    Gate->>MoveIt: dispatch fallback grasp at box location
  end
```
*The VLA and OWL-ViT ground the same instruction independently; the gate accepts only when they agree, else it falls back to the detector's target.*
