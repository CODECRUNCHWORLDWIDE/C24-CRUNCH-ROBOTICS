...
```mermaid
flowchart LR
  R["Reference r"] --> FF["Feedforward model"]
  R --> CMP["Compare to measurement"]
  Y["Measurement y"] --> CMP
  CMP --> FB["PID feedback"]
  FF --> SUM["Sum"]
  FB --> SUM
  ...
  PLANT --> Y
```
*Feedforward predicts the command from the reference while feedback only cleans up the residual error.*
...
```mermaid
flowchart LR
  OUTER["Outer loop position or heading"] --> SETPOINT["Sets inner loop setpoint"]
  SETPOINT --> INNER["Inner loop velocity PID fast"]
  INNER --> PLANT["Plant wheel or joint"]
  PLANT --> INNERFB["Measured velocity"]
  INNERFB --> INNER
  PLANT --> OUTERFB["Measured position"]
  OUTERFB --> OUTER
```
*An outer loop sets the setpoint for a faster inner loop, each tuned independently.*
...
Next: the exercises put all of this on your robot — tune a plant to spec, reproduce-and-fix wind-up and kick, and close a real yaw-rate loop with feedforward. Continue to [the exercises](../exercises/README.md).
