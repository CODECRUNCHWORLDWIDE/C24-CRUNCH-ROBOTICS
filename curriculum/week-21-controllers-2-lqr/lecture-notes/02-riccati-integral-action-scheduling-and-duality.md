...
```mermaid
flowchart TD
  Solve["Solve CARE for P"] --> Check1{"Is P symmetric and positive definite"}
  Check1 -->|No| Stop1["Stop - fix Q or the model"]
  Check1 -->|Yes| Check2{"Closed loop eigenvalues all negative real part"}
  Check2 -->|No| Stop2["Stop - do not deploy"]
  Check2 -->|Yes| Trust["Trust the gain K"]
```
*The three sanity checks as a gate: no gain reaches the robot without passing both.*
...
```mermaid
flowchart TD
  Start{"Multivariable or strongly coupled"} -->|No| PID["Use PID single loop"]
  Start -->|Yes| Model["Linearize to get A and B"]
  Model --> CheckC{"Controllability rank equals n"}
  CheckC -->|No| StopC["Stop and understand the mode"]
  CheckC -->|Yes| Cost["Design Q and R Brysons rule"]
  Cost --> SolveK["Solve Riccati for K then verify"]
  SolveK --> Extra{"Hard constraints present"}
  Extra -->|Yes| MPC["Next week MPC"]
  Extra -->|No| Ship["Ship as LQR ros2_control plugin"]
```
*The same triage rendered as a diagram: PID, LQR, or MPC, gated by coupling, controllability, and constraints.*
