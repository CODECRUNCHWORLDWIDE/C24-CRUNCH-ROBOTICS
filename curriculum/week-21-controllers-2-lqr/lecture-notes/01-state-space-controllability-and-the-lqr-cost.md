...
```mermaid
flowchart LR
  B["B directions now"] --> AB["A times B one step later"]
  AB --> A2B["A squared B two steps later"]
  A2B --> Stack["Stack all columns"]
  Stack --> Rank{"Rank equals n"}
  Rank -->|Yes| Ctrl["Controllable"]
  Rank -->|No| Unctrl["Uncontrollable mode exists"]
```
*How stacking B, AB, A squared B... builds the controllability matrix and its rank test.*
...
```mermaid
flowchart TD
  A["Linearize to get A and B"] --> B["Check controllability rank equals n"]
  B --> C["Design Q with Brysons rule"]
  C --> D["Design R with Brysons rule"]
  D --> E["Next lecture solve Riccati for K"]
```
*The four-ingredient pipeline: model, check, then cost, before any gain is solved.*
