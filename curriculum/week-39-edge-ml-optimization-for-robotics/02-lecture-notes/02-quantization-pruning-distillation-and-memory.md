...
## 6. Knowledge distillation: train a small student

Distillation trains a small "student" network to mimic a large "teacher's" *soft outputs* (the full probability distribution, not just the argmax label). The soft targets carry more information than hard labels — "this is 70% a mug, 25% a cup, 5% a bowl" teaches the student the geometry of the class space — so the student often reaches accuracy that training the small model directly cannot.
...
| 6 | Distillation trains a small student on soft labels; composes with INT8; the Depth-Anything-v2 case. |
