...
We are going to do this in the order the bugs appear. First we build a `Spin90Degrees` action server the obvious way, run it under the default single-threaded executor, send a cancel, and watch it deadlock. Then we will understand *exactly* why, fix it with a callback group, and only then talk about lifecycle. The deadlock is not a corner case — it is the single most common bug in learner (and plenty of professional) action servers, and the fix is three lines once you understand the model.
...
```mermaid
sequenceDiagram
  participant ExecuteThread
  participant ImuThread
  participant CancelThread
  loop 50 Hz control loop
    ExecuteThread->>ExecuteThread: Check is_cancel_requested
  end
  ImuThread->>ExecuteThread: Fresh yaw reading
  CancelThread->>ExecuteThread: Set cancel flag accepted
  ExecuteThread->>ExecuteThread: Stop robot and return canceled
```
*Three reentrant threads run alongside execute so the cancel flag can be set and seen without waiting for the control loop to finish.*
...
stateDiagram-v2
  ...
  Finalized --> [*]
```
*The lifecycle node state machine: an external supervisor drives configure, activate, deactivate, and cleanup.*
