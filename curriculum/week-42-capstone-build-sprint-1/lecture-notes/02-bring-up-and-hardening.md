...
```mermaid
flowchart TD
  A["Power confirmed"] --> B["Safety E-stop verified"]
  B --> C["Signals sensors and TF confirmed"]
  C --> D["Motion lowest authority first"]
```
*Bring-up proceeds bottom-up through four gates, and each layer must pass before the next one is trusted.*
...
```
*A lifecycle node moves through explicit states, so a launch graph can configure every node before activating any of them in order.*
