...
```mermaid
flowchart TD
  A["Odometry error"] --> B["Systematic"]
  A --> C["Non systematic slip"]
  A --> D["Quantization"]
  A --> E["Finite rate integration"]
  B --> B1["Repeatable - calibrate it away"]
  C --> C1["Random - model as noise"]
  D --> D1["Encoder resolution - sets noise floor"]
  E --> E1["Software choice - pick exact arc integrator"]
```
*The four error classes and how each one is handled.*
...
```mermaid
flowchart TD
  M["map frame - accurate but jumps"] -->|"corrected by SLAM AMCL Week 7"| O["odom frame - smooth but drifts"]
  O -->|"published by your odometry node this week"| B["base_link"]
```
*REP-105 splits the drift problem across two composable transforms.*
