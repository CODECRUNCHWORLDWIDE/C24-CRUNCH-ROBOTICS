...
```mermaid
flowchart LR
  N["Nav2 planner - unicycle Twist"] --> C["Base specific controller"]
  C --> H["Hardware wheel speeds or steering angle"]
  H -->|"encoder feedback"| C
  C --> E["nav_msgs Odometry to SLAM and EKF"]
```
*The Twist interface is the model-agnostic seam between Nav2 and the base-specific controller.*
...
```mermaid
flowchart TD
  Q["What wheels does the chassis have"] --> W1["Two driven wheels common axle plus casters"]
  Q --> W2["Steered front wheels driven rear no spin in place"]
  Q --> W3["Four wheels with angled rollers need to strafe"]
  Q --> W4["Three or four omni wheels symmetric layout"]
  W1 --> D1["Diff drive - heading dominated drift"]
  W2 --> D2["Bicycle for odometry Ackermann for steering"]
  W3 --> D3["Mecanum - worst drift of the five"]
  W4 --> D4["Omnidirectional - same caveat as mecanum"]
```
*A wheel layout walks you straight to the model and its dominant drift source.*
