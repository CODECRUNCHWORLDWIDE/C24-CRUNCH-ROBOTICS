...
```mermaid
flowchart TD
  A["Antipodal contact pair"] --> B["Grasp point: midpoint of A and B"]
  A --> C["Closing axis: line from A to B"]
  D["Approach axis: perpendicular safe direction"] --> E["Rotation matrix R"]
  C --> E
  B --> F["Grasp pose: position plus orientation"]
  E --> F
  F --> G["Pre-grasp standoff pose"]
```
*Three geometric facts, the grasp point, closing axis, and approach axis, combine into a full SE3 pose.*
...
stateDiagram-v2
  [*] --> PreGrasp
  PreGrasp --> Approaching : straight line move fingers open
  Approaching --> AtGraspPose : straddling the closing axis
  AtGraspPose --> FingersClosed : close to contact width
  FingersClosed --> Retreating : lift along approach axis
  Retreating --> [*] : confirm object held
```
*A grasp is a small trajectory of waypoints, not a single pose, from standoff through close to retreat.*
