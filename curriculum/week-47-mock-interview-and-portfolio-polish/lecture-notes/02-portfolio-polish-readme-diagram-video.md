...
```mermaid
flowchart TD
  A["What is this and why"] --> B["Architecture diagram"]
  B --> C["Quickstart"]
  C --> D["Results with numbers"]
  D --> E["Limitations"]
```
*The README section order mirrors the sequence of questions a reviewer actually asks.*
...
```mermaid
flowchart LR
  A["Project 1 - Perception cycle - Foundation"] --> B["Project 2 - Policy plus fallback - Judgment"]
  B --> C["Project 3 - Capstone - Synthesis"]
```
*Three flagship projects, escalating into one trajectory rather than three unrelated demos.*
