...
```mermaid
flowchart TD
  A["Prior mean and covariance"] --> B["Predict: unscented transform through motion model g"]
  B --> C["Predicted mean and covariance"]
  C --> D["Push predicted sigma points through measurement model h"]
  D --> E["Compute innovation covariance S and cross covariance Pxz"]
  E --> F["Kalman gain K equals Pxz times S inverse"]
  F --> G["Corrected mean and covariance"]
  G --> A
```
*One full UKF predict-update cycle: sigma points replace Jacobians at every stage, and the corrected belief feeds the next predict.*
...
```mermaid
flowchart TD
  A["Particle cloud from previous step"] --> B["Sample: apply motion model with noise"]
  B --> C["Weight: score each particle by measurement likelihood"]
  C --> D["Normalize weights to sum to one"]
  D --> E["Resample: draw N particles proportional to weight"]
  E --> A
```
*The SIR loop, repeated every timestep: sample, weight, normalize, resample.*
