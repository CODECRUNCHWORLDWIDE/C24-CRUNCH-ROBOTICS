...
```mermaid
flowchart LR
  P["Prior factor"] --> X0["x0"]
  X0 --> O1["Between factor odom"] --> X1["x1"]
  X1 --> O2["Between factor odom"] --> X2["x2"]
  X0 --> LC["Between factor loop closure"]
  X2 --> LC
```
*A pose graph: one prior anchors x0, odometry chains the poses, and a loop closure adds an extra constraint between distant poses.*
...
```mermaid
flowchart TD
  A{"Need to revisit past states"} -->|No| B{"Belief unimodal and Gaussian"}
  A -->|Yes| C["Build a factor graph"]
  B -->|Mild nonlinearity| D["EKF"]
  B -->|Strong nonlinearity| E["UKF"]
  B -->|Multimodal| F["Particle filter or AMCL"]
  C --> G{"Online large or incremental"}
  G -->|No| H["Levenberg-Marquardt"]
  G -->|Yes| I["iSAM2"]
```
*The estimation-method decision tree: filter vs smoother first, then the specific estimator.*
