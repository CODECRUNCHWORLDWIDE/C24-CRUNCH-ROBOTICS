...
```mermaid
flowchart LR
  A["Left camera image"] --> C["Match patch left to right"]
  B["Right camera image"] --> C
  C --> D["Disparity d"]
  D --> E["Depth Z from d, focal length, and baseline"]
```
*The stereo depth pipeline: matching gives disparity, disparity gives depth.*
...
```mermaid
flowchart TD
  A["Depth camera technologies"] --> B["Triangulation based"]
  A --> C["Phase based"]
  B --> D["Passive stereo"]
  B --> E["Active stereo"]
  B --> F["Structured light"]
  C --> G["Time of flight"]
```
*The four technologies split into triangulation methods and the one phase-shift method, ToF.*
...
5. **Budget and ecosystem.** RealSense has the most mature ROS2 driver and the largest community; that alone is often decisive for a learner or a small team.
