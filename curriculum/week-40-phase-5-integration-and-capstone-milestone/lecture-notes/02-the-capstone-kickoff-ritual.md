...
```mermaid
flowchart TD
  A["Sensors and bridge"] --> B["State estimation"]
  B --> C["Perception"]
  C --> D["Planning and control"]
  D --> E["Policy and safety"]
  E --> F["Behavior tree and telemetry"]
```
*Ordered lifecycle bring-up: nothing activates until the stage below it is active.*
...
```mermaid
flowchart TD
  A["Bring up the graph"] --> B["Run pre-flight check"]
  B -->|Pass| C["Start the capstone run"]
  B -->|Fail| D["Abort and tear down"]
```
*The gate is only real if a failed check stops the run rather than just reporting it.*
