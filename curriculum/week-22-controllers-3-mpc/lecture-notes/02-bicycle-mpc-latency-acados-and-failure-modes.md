...
```mermaid
flowchart TD
  A["MPC misbehaving"] --> B["Status not optimal"]
  A --> C["Status optimal but poor tracking"]
  A --> D["Drove into a trap"]
  A --> E["Fine in sim bad on hardware"]
  B --> F["Soften constraints or fall back to safe controller"]
  C --> G["Lengthen horizon or raise effort weight"]
  D --> H["Add a terminal set or longer horizon"]
  E --> I["Reprofile latency or check model mismatch"]
```
*The failure-mode decision tree collapsed to its four branches: bad status, poor tracking, a trap, or a sim-to-hardware gap.*
...
```mermaid
flowchart LR
  A["Path or goal"] --> B["MPC outer loop - solves QP at about 20 Hz"]
  B --> C["Reference - velocity and curvature"]
  C --> D["Inner controller - PID or LQR at high rate"]
  D --> E["Motors"]
```
*The MPC runs as a slow outer loop emitting a reference that a fast inner controller tracks smoothly.*
...
Next: the exercises put all of this on your robot — formulate an MPC and verify it equals LQR, watch constraints bind, and track a figure-8 with a profiled bicycle MPC. Continue to [the exercises](../exercises/README.md).
