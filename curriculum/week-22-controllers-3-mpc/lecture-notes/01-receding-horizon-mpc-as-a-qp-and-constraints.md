...
```mermaid
flowchart TD
  A["Predict N steps ahead"] --> B["Optimize the input sequence"]
  B --> C["Apply only u0"]
  C --> D["Advance one step"]
  D --> A
```
*The receding-horizon loop: predict, optimize, apply one input, then re-solve from the new state.*
...
```mermaid
flowchart TD
  A["Safety critical limit"] --> B["Hard constraint - forbids violation"]
  C["Preferred but not critical"] --> D["Soft constraint - slack variable plus penalty"]
  B --> E["Solver rejects any solution that crosses it"]
  D --> F["Solver allows crossing it but pays heavily"]
```
*Hard constraints forbid; soft constraints discourage with a penalty and keep the QP feasible.*
