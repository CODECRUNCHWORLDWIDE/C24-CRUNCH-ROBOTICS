...
```mermaid
flowchart LR
  A["On Playback Tick"] --> B["Isaac Read LiDAR"]
  B --> C["ROS2 Publish LaserScan to /scan"]
  D["Read Sim Time"] --> E["ROS2 Publish Clock to /clock"]
  F["Read TF prims"] --> G["ROS2 Publish Transform Tree to /tf"]
```
*The OmniGraph dataflow that bridges an Isaac Sim LiDAR prim and clock and TF onto ROS2 topics.*
...
```mermaid
flowchart TD
  Q["What am I doing with this sim right now"] --> A["Building or debugging the autonomy stack"]
  Q --> B["Training a policy needing millions of samples or domain randomization"]
  Q --> C["Generating photorealistic images for a vision policy"]
  Q --> D["A quick physics or contact sanity check"]
  A --> A1["Use Gz Sim - free ROS native fast iteration"]
  B --> B1["Use Isaac Lab - GPU parallel throughput"]
  C --> C1["Use Isaac Sim - RTX closes the visual gap"]
  D --> D1["Use Gz Sim - swap the engine to probe contacts"]
```
*The one-question decision tree for picking a simulator by job.*
