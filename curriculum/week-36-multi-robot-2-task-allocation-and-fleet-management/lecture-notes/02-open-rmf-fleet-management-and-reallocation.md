...
stateDiagram-v2
  ...
  canceled --> [*]
```
*The task lifecycle the dispatcher tracks, and that reallocation logic keys off.*
...
```mermaid
flowchart TD
  A["Robot stops reporting"] --> B["Staleness timeout - no state update"]
  C["Robot reports but makes no progress"] --> D["Progress or ETA check fails"]
  B --> E["Task marked orphaned"]
  D --> E
  E --> F["Dispatcher re-bids the task among healthy robots"]
  F --> G["A healthy robot is reassigned the task"]
```
*Two failure modes, comms death and wedged-but-alive, both funnel into re-bidding the orphaned task.*
...
Next: the exercises put the allocation math on a running graph and the fleet manager on your robots. Continue to [the exercises](../exercises/README.md).
