...
```mermaid
flowchart TD
    Root["NavigateRecovery"] --> Pipeline["PipelineSequence Navigate with Replanning"]
    Root --> Fallback["ReactiveFallback Recovery"]
    Pipeline --> Rate["RateController 1 Hz"]
    Rate --> Plan["ComputePathToPose"]
    Pipeline --> Follow["FollowPath"]
    Fallback --> Goal["GoalUpdated bails out"]
    Fallback --> Robin["RoundRobin"]
    Robin --> Actions["Clear costmaps then Spin then Wait then BackUp"]
```
*The default tree's shape: a replanning pipeline wrapped by a round-robin recovery fallback.*
...
```mermaid
stateDiagram-v2
    [*] --> Running: onRun starts the hold
    Running --> Running: onCycleUpdate hold still active
    Running --> Succeeded: onCycleUpdate hold cleared
    Succeeded --> [*]
```
*OperatorHold's own lifecycle each time the behavior_server ticks it.*
...
Next: the exercises put all of this on your week-7 map. Continue to [the exercises](../exercises/README.md).
