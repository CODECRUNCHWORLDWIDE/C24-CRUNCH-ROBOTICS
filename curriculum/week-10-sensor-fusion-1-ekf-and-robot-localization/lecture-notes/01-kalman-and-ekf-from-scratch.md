...
```mermaid
flowchart LR
  A["Belief mean and covariance"] --> B["Predict step grows uncertainty by Q"]
  B --> C["New measurement arrives"]
  C --> D["Compute innovation and Kalman gain K"]
  D --> E["Update step shrinks uncertainty using K"]
  E --> A
```
*The Kalman filter's endless loop: predict adds uncertainty, update removes it.*
...
```mermaid
flowchart TD
  A["Compare the filters stated uncertainty to its actual error"] --> B{"Does the actual error stay inside the P envelope"}
  B -->|Yes| C["Filter is consistent"]
  B -->|Error much smaller than P| D["Underconfident: Q too large or an R too small filter jitters"]
  B -->|Error much larger than P| E["Overconfident: Q too small or an R too small filter drifts confidently"]
  E --> F["Escalate to Unscented KF or factor graph"]
```
*Reading the filter's own covariance tells you which of the three failure modes you're in.*
