...
```mermaid
flowchart TD
  A["Collect demos"] --> B["Split train and val"]
  B --> C["Fit normalization on train only"]
  C --> D["Train one epoch"]
  D --> E["Evaluate validation loss"]
  E --> F{"Val loss improved"}
  F -- "Yes" --> G["Save best model reset patience"]
  F -- "No" --> H["Increment patience counter"]
  G --> I{"Epochs left and patience ok"}
  H --> I
  I -- "Continue" --> D
  I -- "Stop" --> J["Load best model"]
```
*The BC training loop: split and normalize once, then train-evaluate-checkpoint until patience runs out.*
...
```mermaid
flowchart TD
  A["Policy misbehaves"] --> B{"Alignment normalization and action rep all correct"}
  B -- "No" --> C["Fix the mundane bug found"]
  B -- "Yes" --> D{"Loss curves healthy"}
  D -- "No" --> E["Underfit or overfit tune model or data"]
  D -- "Yes" --> F{"Fails everywhere or only after drifting"}
  F -- "Everywhere" --> G["Deploy bug or underfit"]
  F -- "Drifts after good start" --> H["Covariate shift run DAgger"]
```
*The debugging order: rule out mundane bugs before reaching for the covariate-shift diagnosis.*
