...
```mermaid
flowchart LR
  A["On-distribution state"] --> B["Policy acts slightly wrong"]
  B --> C["Slightly off-distribution next state"]
  C --> D["Less training data here so more wrong"]
  D --> E["Further off-distribution state"]
  E --> F["Even more lost"]
  F -->|"repeat"| C
```
*The compounding-error chain: each small error pushes the policy into states it has less data about, which produces bigger errors.*
...
```mermaid
flowchart TD
  A["Train initial policy on expert demos"] --> B["Roll out current policy in the environment"]
  B --> C["Record the states it visits"]
  C --> D["Query the expert for the correct action at each visited state"]
  D --> E["Aggregate new pairs into the dataset"]
  E --> F["Retrain policy on the aggregated dataset"]
  F -->|"next round"| B
  F -->|"enough rounds"| G["Deploy the leashed policy"]
```
*The DAgger loop: the policy supplies the states it visits, the expert supplies the labels, and the dataset only ever grows.*
