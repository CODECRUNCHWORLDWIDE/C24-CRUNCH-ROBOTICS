...
```mermaid
flowchart TD
  A["Deadlock detected"] --> B["Rung 1 relax constraints and replan"]
  B -- Success --> F["Resume task"]
  B -- Still stuck --> C["Rung 2 clear costmap and re-perceive"]
  C -- Success --> F
  C -- Still stuck --> D["Rung 3 request operator assist and wait"]
  D -- Assist arrives --> F
  D -- No assist --> E["Rung 4 controlled stop task blocked"]
```
*Escalate only when the cheaper rung fails — controlled stop is the last resort, not the first response.*
