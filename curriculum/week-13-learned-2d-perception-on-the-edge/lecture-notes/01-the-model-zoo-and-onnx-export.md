...
```mermaid
flowchart LR
  A["PyTorch model trained"] --> B["Export to ONNX graph"]
  B --> C["ONNX portable artifact"]
  C --> D["Runtime consumes ONNX"]
  D --> E["Fast engine runs on device"]
```
*PyTorch trains, ONNX transports, the runtime runs — three distinct jobs, not one.*
...
```mermaid
flowchart LR
  A["Raw tensor 84 by 8400"] --> B["Transpose to anchor rows"]
  B --> C["Threshold class scores"]
  C --> D["Non max suppression"]
  D --> E["Rescale to image pixels"]
```
*Turning the raw detection tensor into boxes you can publish, one gotcha per arrow.*
